...
public class Feed {
    ...
    func checkBasicFaviconURL(url: URL) {
        ...
        URLSession.shared.getReturnedDataFrom(url: url, completion: { (data, responce, error) in
...
//                                                userInfo: [errorInfoKey:foundError])
            ...
            if let headers: URLResponse = responce,
                ...
                //TODO: replace with protocol method call
...
//                                                    userInfo: [errorInfoKey:unrecognizableDataError])
        ...
        func requestUpdatedStories() {
            unowned let unownedSelf: Feed = self
            URLSession.shared.getReturnedDataFrom(url: url, completion: { (data, respone, error) in
                if let foundError: Error = error {
                    //TODO: replace with protocol callback
